...
source("/Users/timakpo/Desktop/CC-Template/Paper Submitt/Paper-LPR/Revision/Part1.R")
...
# Estimation of global model parameters (Table 8) from "Likelihood_2exchangeable". 
...
kappa2=exp(para[8])/(1+exp(para[8]))
kappa1=exp(para[9])/(1+exp(para[9])); delta3=exp(para[10])/(1+exp(para[10]))


## Prediction : Figure 5 obtained with the "Prediction_2_exchangeable" function
# on two schools (i=1 and i=30)
...
# school 30
Pred_School1<-Prediction_2_exchangeable(i=30,K=10,data=data.sch,alpha1=alpha1,beta1=beta1,
                                        alpha2=alpha2,beta2=beta2,lambda=lambda,delta2=delta2,
                                        kappa1=kappa1,kappa2=kappa2,delta3=delta3)

xpred30<-Pred_School1$xpred; ypred30<-Pred_School1$ypred
...
## Figure 6 curves with "Conditionnal_density" output 

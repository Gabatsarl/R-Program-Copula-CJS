...
Likelihood_2exchangeable<-function(para,data){
  ...
  for(i in 1:m){
    ...
    #ind<-which(data[,1]==i)
    #lnC1<--dCopula(u0[ind],normalCopula(rho1, dim=length(which(data[,1]==i))),log=TRUE)
    l01<-l01-dCopula(u0[ind],normalCopula(rho1, dim=length(which(data[,1]==i))),log=TRUE)
  }
  ...
  for(i in 1:m){
    ...
    #lnC3<- -dCopula(w0[ind1],normalCopula(rho3,dim=length(which(data[,1]==i))),log=TRUE)
    l03<-l03-dCopula(w0[ind1],normalCopula(rho3,dim=length(which(data[,1]==i))),log=TRUE)
  }
...
Prediction_2_exchangeable=function(i,K,data,alpha1,beta1,alpha2,beta2,lambda,delta2,kappa1,kappa2,delta3){
  ...
  w0<-1-(1-kappa1)*(1-u0)^(-kappa1)*(1-v0)^(1-kappa2)*z-
      kappa1*(1-v0)^(1-kappa2)*BiCopHfunc1(u1=(1-u0)^kappa1,u2=(1-v0)^kappa2, family=1, par=delta2)
